                    ...
                    <form action="{{ route('admin.user.update', $user->id) }}" method="post" enctype="multipart/form-data">
                      @csrf {{ method_field('PUT') }}
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="errorName" value="{{ $user->name }}" class="form-control">
                            <p class="text-danger" id="errorsName">{{ $errors->first('name') }}</p>
                        </div>
                        <div class="form-group">
                            <label for="email">Email</label>
                            <input type="email" name="email" id="errorEmail" value="{{ $user->email }}" class="form-control">
                            <p class="text-danger" id="errorsEmail">{{ $errors->first('email') }}</p>
                        </div>
                        <div>
                            <label for="password">Password</label>
                            <input type="password" name="password" id="errorPsw" value="{{ $user->password }}" class="form-control">
                            <p class="text-danger" id="errorsPsw">{{ $errors->first('password') }}</p>
                        </div>
                        <div class="custom-control custom-switch mt-3">
                            <input type="checkbox" name="role" {{ $user->role ? "checked" : "" }} class="custom-control-input" id="customSwitch1">
